 ...
 * Time: 8:44 AM
 */
public class TestFiller2 {

    double eraseSpace=0.0125;

    public TestFiller2() throws Exception {
        new ATrack(""){
            @Override
...
//                dc.eraseTo(Point.fromRT(1, startTheta));
                ...
                TreeSet<Double> drawnThetas = new TreeSet<>();
                double minTheta=.04;

                while (rho<=1){
                    boolean ready=theta>4*2*Math.PI;

                    theta+= !ready ? .04 : .001/rho;
                    ...
                    Double lowerTheta=drawnThetas.lower(theta);
                    Double higherTheta=drawnThetas.higher(theta);

                    if (lowerTheta!=null && rho*(theta-lowerTheta)<minTheta){
                        continue;
                    }

                    if (higherTheta!=null && rho*(higherTheta-theta)<minTheta){
                        continue;
                    }
...
//                    dc.lineTo(dc.getCurrentRelativePosition().vectorTo(Point.fromRT(rho, theta)));
                    ...
                    Point tempPoint=Point.fromRT(rho-eraseSpace, theta);
                    int fillNumber2=getFill(tempPoint.rho, tempPoint.theta, pixels, height, width);

                    if (fillNumber==0 && fillNumber2==1){
                        while (fillNumber2==1){
                            tempPoint=Point.fromRT(tempPoint.rho-eraseSpace, tempPoint.theta);
    ...
    public static void main(String args[]) throws Exception {
        TestFiller2 me = new TestFiller2();
    }
}